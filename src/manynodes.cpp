...
int main(int argc, char **argv) {
    ...
    for (auto itr = us_cities.begin(); itr != us_cities.end(); ++itr) {
        
        string city_id = itr->getCity() + ", " + itr->getState();
        city_graph.addVertex(city_id);
         if (itr != us_cities.begin()) {
            auto prev = itr - 1;
            string prev_id = prev->getCity() + ", " + prev->getState();
            city_graph.addEdge(city_id, prev_id);
         }
    }
